...
export const postlike = async (req, res) => {
    ...
    try {
        ...
        res.json({ message: "Post liked successfully", likes: post.likes.Totallike });
    } catch (error) {
...
export const postunlike = async (req, res) => {
    ...
    try {
        ...
        if (post.likes.userId.includes(userId)) {
            // Increase like count and add user to likedBy array
            post.likes.Totallike -= 1;
            post.likes.userId = post.likes.userId.filter(id => id.tostring() !== userId.tostring());
            await post.save();

            res.json({ message: "Post Unliked successfully", likes: post.likes.Totallike });
        }
    } catch (error) {
        res.status(500).json({ message: "Internal server error", error });
    }
...
export const addcomment = async (req, res) => {
    try {
        ...
        if (!postId || !userId || !text) {
            return res.status(400).json({ message: "Post ID, User ID, and text are required." });
        }
...
export const addreply = async (req,res)=>{ 
    try {
      ...
      if (!postId || !commentIndex || !userId || !text) {
        return res.status(400).json({ message: "Post ID, Comment Index, User ID, and text are required." });
      }
  ...
  export const updatereply = async (req,res)=>{ 
    try {
      ...
      if (!text) {
        return res.status(400).json({ message: "Text for the reply is required." });
      }
